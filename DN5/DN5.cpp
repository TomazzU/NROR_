...
int main() {
    ...
    // podatke iz string-a, èe so locirani z ' '
    ...
    // iteracijo, da preberemo in zapolnimo matriko A
    ...
    // zato jo samo preberemo in ne naredimo nièesar. Poglejte v
    ...
    int n_b; // sem bomo dali velikost vektorja b (ki je identièna velikosti A)
    ...
    // inicializiramo vektor rešitve T, npr. na 80 stopinj.
    vector<double> T(n_b, 80.0); // Initialize T with the size of n_b

    auto start_time = std::chrono::high_resolution_clock::now();

    for (int ii = 0; ii < 2000; ii++) {
        // Izraèun Gauss-Seidelove metode
        auto start_time_gs = std::chrono::high_resolution_clock::now();
...
#pragma omp parallel for shared(T, A, b) // Paralelizacija z OpenMP
        ...
        auto end_time_gs = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> time_duration_gs = end_time_gs - start_time_gs;
